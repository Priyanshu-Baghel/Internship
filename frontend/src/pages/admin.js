...
const Admin = ({ gmapsApiKey }) => {
  ...
  const fetchVehicles = async () => {
    ...
    fetch(`${config.API_BASE_URL}/vehicles`)
      .then(res => res.json())
      .then(jsonResponse => {
        console.log("console log the list of vehicles", jsonResponse); // Log the response to the console
        setVehicles(jsonResponse.filter(vehicle => vehicle.pos));
        // setVehicles(jsonResponse);
        console.log("This is the list of vehicles",vehicles);
      })
      .catch(err => {
  ...
  return (
    ...
    <VehicleList setCurrVehiclePath={handleVehicleSelect} fetchVehicles={fetchVehicles} vehicles={vehicles} map={map} isLoaded={isLoaded} />
      {!isLoaded ? <Skeleton component="div" variant="rectangular" sx={{ minHeight: 'inherit', width: '100%',height:"screen" }} /> : (
        <GoogleMap
          center={center}
          zoom={15}
          options={{ streetViewControl: true, mapTypeControl: true, fullscreenControl: true }}
          mapContainerStyle={{ minHeight: '500px' }}
